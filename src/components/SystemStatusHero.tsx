    ...
    switch (status) {
      ...
      case 'PROTECTION ACTIVE':
        return {
          ...
          icon: Flame,
          summary: 'High-Altitude Thermal Protection Active: PTC heating element operating to prevent equipment freeze.',
        };
      case 'NORMAL':
      default:
        return {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 bg-slate-950/70 p-3 rounded-xl border border-slate-800/80 font-mono text-xs">
          <div className="p-2 rounded bg-slate-900/60 border border-slate-800/60">
            <div className="text-[10px] text-slate-400 uppercase">Heater</div>
            <div className="font-bold flex items-center gap-1.5 mt-0.5">
              <Flame className={`w-3.5 h-3.5 ${heaterStatus === 'ON' ? 'text-rose-400' : 'text-slate-500'}`} />
              <span className={heaterStatus === 'ON' ? 'text-rose-400' : 'text-slate-300'}>
                {heaterStatus} ({protection.heaterMode})
              </span>
            </div>
          </div>

          <div className="p-2 rounded bg-slate-900/60 border border-slate-800/60">
            <div className="text-[10px] text-slate-400 uppercase">Hazard Index</div>
            <div className="font-bold flex items-center gap-1.5 mt-0.5">
              <Activity className="w-3.5 h-3.5 text-cyan-400" />
              <span className="text-white">{assessment.hazardScore} / 100</span>
            </div>
          </div>

          <div className="p-2 rounded bg-slate-900/60 border border-slate-800/60">
            <div className="text-[10px] text-slate-400 uppercase">Protection Loop</div>
            <div className="font-bold flex items-center gap-1.5 mt-0.5">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
              <span className={protection.systemProtection === 'ACTIVE' ? 'text-cyan-300' : 'text-slate-300'}>
